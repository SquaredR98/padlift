...
export function FaqTwoColumnStatic({ branding, content, styles }: BlockComponentProps) {
  ...
  const renderFaq = (faq: { question: string; answer: string }, idx: number) => (
    <details key={idx} className="group border-b border-[var(--lp-site-border)] py-4">
      <summary className="flex cursor-pointer items-center justify-between text-sm font-medium text-[var(--lp-site-heading)]">
        {faq.question}
